...
void replay_gmn_copy(UInt_t runnum=10491, Long_t nevents=-1, Long_t firstevent=0, const char *fname_prefix="e1209019", UInt_t firstsegment=0, UInt_t maxsegments=1, Int_t pedestalmode=0, Int_t cmplots=0)
{
  ...
  if( nevents > 0 ){ 
    ...
    outfilebase.Form( "%s_replayed_copy_%d_stream%d_seg%d_%d_firstevent%d_nevent%d", fname_prefix, runnum,
		      stream, firstsegment, lastsegment, firstevent, nevents );
    outfilename.Form( "%s/%s.root", prefix.Data(), outfilebase.Data());
  } else {
    outfilebase.Form( "%s_fullreplay_copy_%d_stream%d_seg%d_%d", fname_prefix, runnum,
